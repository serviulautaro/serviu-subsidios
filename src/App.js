    ...
    try {
      ...
      for (const p of zipSeleccionados) {
        ...
        for (const nombre of archivosSet) {
          ...
          const url = apiPath("/files/", rutaArch, nombre);
          try {
        ...
        {archivos.length === 0 ? (
          ...
          <div style={{ position: "relative" }} onClick={e => { if (!e.target.closest("[data-docmenu]")) setDocMenu(null); }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
              {archivos.map(arch => {
                const fileUrl = apiPath("/files/", archivosRutas[arch] || carpeta, arch);
              ...
              {sol.documentos.map((doc, i) => {
                ...
                if (esCustom) {
                  ...
                  const guardarArchivoDoc = async (fileOrName) => {
                    const nombreArchivo = typeof fileOrName === "string" ? fileOrName : fileOrName.name;
                    const nuevasSols = solicitudes.map(s => s.id !== sol.id ? s : {
                      ...s,
                      documentos: s.documentos.map((d, j) => j === i ? { ...d, archivo: nombreArchivo } : d)
                    });
                    onSaveSolicitudes(nuevasSols);
                    await supabase.from("solicitudes").update({ documentos: nuevasSols.find(s=>s.id===sol.id).documentos }).eq("id", sol.id);
                  };
                            ...
                            <input type="file" style={{ display: "none" }} accept=".pdf,.jpg,.jpeg,.png"
                              onChange={async e => {
                                ...
                                try {
                                  ...
                                  onSaveSolicitudes(solicitudes.map(s => s.id !== sol.id ? s : { ...s, documentos: s.documentos.map((d2, i2) => i2 !== i ? d2 : { ...d2, archivo: nombreSubido, valor: newValor }) }));
                              ...
                              <input type="file" style={{ display: "none" }} accept=".pdf,.jpg,.jpeg,.png"
                                onChange={async e => {
                                  ...
                                  try {
                                    ...
                                    const nuevasSols = solicitudes.map(s => s.id !== sol.id ? s : {
                                      ...s, documentos: s.documentos.map((d2, i2) => i2 === i ? { ...d2, archivo: nombreSubido, entregado: true } : d2)
                                    });
                                    onSaveSolicitudes(nuevasSols);
                                  } catch (err) { alert("Error al subir archivo: " + (err.message || "")); }
                                  finally { e.target.value = ""; }
