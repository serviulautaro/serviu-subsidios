...
function DetallePersona({ personaId, personas, solicitudes, comites, programasCustom, onBack, onSaveSolicitudes, onSavePersonas, onCargarSolicitudesPersona, currentUser, registrarAuditoria }) {
  ...
  // Update via servidor Render (más confiable que Supabase directo en producción)
  const updateViaServidor = async (tabla, filtros, valores) => {
    try {
      ...
      console.warn("[updateViaServidor]", tabla, json.error || res.status);
      return false;
    } catch (e) {
      console.warn("[updateViaServidor fetch]", e.message);
      return false;
    }
  };
  ...
  const guardarFechaVisitaDesmarque = async (sol, fecha) => {
    ...
    // Sincronizar via servidor Render (directo a PostgreSQL, sin CORS)
    const sincronizar = async () => {
      const filtros = [{ col: "id", value: sol.id }];
      const valores = { documentos: JSON.stringify(documentos), fecha_visita: fecha || "" };

      // Intento 1: servidor Render (más confiable)
      for (let i = 0; i < 3; i++) {
        if (i > 0) await new Promise(r => setTimeout(r, 2000));
        const ok = await updateViaServidor("solicitudes", filtros, valores);
        if (ok) {
          try {
            const pendientes = JSON.parse(localStorage.getItem(LS_FECHAS_KEY) || "{}");
      ...
      // Intento 2: Supabase directo como fallback
      try {
        const { error } = await supabase.from("solicitudes")
          .update({ documentos, fecha_visita: fecha || null }).eq("id", sol.id);
        if (!error) {
          try {
            const pendientes = JSON.parse(localStorage.getItem(LS_FECHAS_KEY) || "{}");
            delete pendientes[sol.id];
    ...
    const nuevasSols = solicitudes.map(s => String(s.id) !== String(solId) ? s : {
      ...s, documentos: s.documentos.map((d, i) => {
        ...
        if (tipoReal === "agua" && opcion !== "Pozo") {
          ...
          const nombreBase = ["pozo", "con arranque"].includes(nombreActual.toLowerCase()) ? "" : nombreActual;
          valor = [nombreBase, partes[1] || "", String(partes[2] || "").trim()].filter((x, i) => i < 2 || x).join("|");
        }
          ...
          <div key={sol.id} style={{ background: "#fff", borderRadius: 14, padding: "22px 26px", marginBottom: 16, border: "1px solid #e8e3de" }}>
            ...
            {solsEditando[sol.id] && <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {(() => {
                ...
                const aprNombreRaw = aprPartes[0] || "";
                const aprNombre = String(aprNombreRaw).trim().toLowerCase() === "con arranque" ? "" : aprNombreRaw;
...
function DetalleComite({ comiteId, comites, personas, solicitudes, programasCustom = [], onBack, onSaveComites, onSavePersonas, onSaveSolicitudes, onDetail, currentUser, registrarAuditoria }) {
  ...
  const programaComiteCsp = (() => {
    const programa = comite?.programaId || comite?.programa_id || "";
    if (["csp_rural", "csp_urbano"].includes(programa)) return programa;
    const tipo = String(comite?.tipo || "").toUpperCase();
    if (tipo === "RURAL") return "csp_rural";
    if (tipo === "URBANO") return "csp_urbano";
    return "";
  })();
  const esComiteCsp = !!programaComiteCsp;
  useEffect(() => {
    if (!esComiteCsp) {
    ...
    let activo = true;
    Promise.all([
      fetch(`${API}/api/db/revisiones_abogado?programa_id=eq.${programaComiteCsp}&orderBy=fecha&orderAsc=false`, { cache: "no-store" }).then(r => r.json()),
    ...
    }).catch(err => console.warn("[detalle comite CSP]", err.message));
    return () => { activo = false; };
  }, [comiteId, esComiteCsp, programaComiteCsp]);
    ...
    <div>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {filtered.map(p => {
          ...
          const solicitudCsp = solsAll.find(s => (s.programaId || s.programa_id) === programaComiteCsp);
          const revisionAbogadoCsp = esComiteCsp ? revisionesAbogadoComite.find(rev =>
            String(rev.persona_id) === String(p.id) &&
            (!solicitudCsp?.id || !rev.solicitud_id || String(rev.solicitud_id) === String(solicitudCsp.id))
          ) : null;
          const estadosRevision = revisionAbogadoCsp ? [revisionAbogadoCsp.dominio_estado, revisionAbogadoCsp.avaluo_estado, revisionAbogadoCsp.ruralidad_estado] : [];
          const resumenRevisionAbogado = !revisionAbogadoCsp ? "NO"
          ...
          const ahorroCompletoCsp = esComiteCsp && solicitanteConAhorroCompleto(p);
          const siguientePasoCsp = esComiteCsp
  ...
  const cargarSolicitudesPersona = async (personaId) => {
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        for (const sol of solicitudesCompletas) {
          const p = pendientesFechas[sol.id];
          if (!p) continue;
          try {
            const res = await fetch(`${API}/api/db/solicitudes/update`, {
