...
function imprimirComite(comite, personas, solicitudes) {
  ...
  const directiva = (comite.directiva || []).map(d => {
    const persona = personas.find(p => (d.rut && rutKey(p.rut) === rutKey(d.rut)) || (d.nombre && norm(p.nombre) === norm(d.nombre))) || {};
    return `<tr><td>${v(d.rol)}</td><td>${v(d.nombre || persona.nombre)}</td><td>${v(d.rut || persona.rut)}</td><td>${v(d.direccion || persona.direccion)}</td><td>${v(d.coordenadas || persona.coordenadas)}</td><td>${v(d.telefono || persona.telefono)}</td><td>${v(d.email || persona.email || persona.correo)}</td></tr>`;
  }).join("");
  imprimirVentana("Informe general", `<div class="page"><div class="top"><div><h1>Unidad de Vivienda</h1><div class="muted">Ilustre Municipalidad de Lautaro</div></div><div style="text-align:right"><h1>Informe general</h1><div class="muted">${new Date().toLocaleDateString("es-CL")}</div></div></div><div class="bar"><span>${v(comite.nombre)}</span><span>${v(comite.tipo)}</span></div><div class="grid">${campoHtml("Constructora", comite.constructora)}${campoHtml("Profesional", comite.profesional)}${campoHtml("Personalidad jurídica", comite.pj)}${campoHtml("Vencimiento", comite.venc)}</div><div class="section"><h2>Directiva</h2>${directiva ? `<table><thead><tr><th>Cargo</th><th>Nombre</th><th>Cédula</th><th>Dirección</th><th>Coordenadas</th><th>Teléfono</th><th>Correo</th></tr></thead><tbody>${directiva}</tbody></table>` : `<div class="muted">Sin directiva registrada.</div>`}</div><div class="section"><h2>Solicitantes</h2><table><thead><tr><th>#</th><th>Nombre</th><th>Cédula</th><th>Teléfono</th><th>Dirección</th><th>Coordenadas</th><th>Documentos</th></tr></thead><tbody>${filas}</tbody></table></div></div>`);
}
