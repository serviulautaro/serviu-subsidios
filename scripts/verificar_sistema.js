...
function main() {
  ...
  ok("Linea de tiempo CSP no reescribe fichas antiguas", contains(app, "soloEstadoLocal") && contains(app, "No volver a enviar fichas completas") && contains(app, "JSON.stringify(payloadAnterior) === JSON.stringify(payload)"));
  ...
  ok("Insercion automatica consulta Render para evitar duplicados", contains(app, "existeSolicitudProgramaEnRender") && contains(app, "eq[persona_id]") && contains(app, "eq[programa_id]") && contains(app, "!solExistenteRender?.id"));
  ...
  ok("Califica para visita no se filtra de la linea", contains(app, "extrasFueraPrograma.filter(d => d?.interno && !d?.ocultoPorPrograma && !d?.duplicadoFusionado)") && contains(app, "docCalificacionDesmarque"));
  ...
  ok("Reparacion deja solo requisitos vigentes y conserva internos reales", contains(app, "const incluirExtras = opciones.incluirExtras === true") && contains(app, "!d?.ocultoPorPrograma && !d?.duplicadoFusionado") && contains(app, "interno: false") && contains(app, "reparar_requisitos_solicitudes_activas") && contains(app, "API + \"/api/db/solicitudes/update\""));
  ...
  ok("Visor abre documentos desde su bucket R2 registrado", contains(app, "archivoGuardado.r2Bucket") && contains(app, "?bucket=${encodeURIComponent(archivoGuardado.r2Bucket)}") && contains(app, "registroPermanente?.r2_key"));
  ...
  ok("Auditoria R2 completa es por lotes y no destructiva", contains(server, "/api/r2/auditar-documentos-lote") && contains(server, "/api/r2/registrar-solicitantes-lote") && contains(server, "_REGISTRO_SOLICITANTE.json") && !contains(server, "DELETE FROM archivos_solicitante"));
  ...
  ok("Listado CSP Urbano muestra los mismos indicadores sin mezclar programas", contains(app, 'if (tipo === "URBANO") return "csp_urbano"') && contains(app, "programaComiteCsp") && contains(app, "solicitudCsp"));
